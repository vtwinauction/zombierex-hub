...
export const unblockUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("user_blocks").delete()
...
export const muteUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("user_mutes")
...
export const upsertKeywordFilter = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("keyword_filters").insert({
...
export const removeKeywordFilter = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("keyword_filters").delete()
...
export const adminResolveReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context.supabase, context.userId);
    const { error } = await context.supabase.from("reports")
      .update({
        status: data.status,
...
export const adminModerateUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const expires_at = data.duration_hours
      ...
      : null;
    const { data: row, error } = await context.supabase.from("moderation_actions").insert({
...
export const adminModerationQueueStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await assertAdmin(context.supabase, context.userId);
    const [pending, reviewing, appeals, active] = await Promise.all([
      context.supabase.from("reports").select("id", { count: "exact", head: true }).eq("status", "pending"),
      context.supabase.from("reports").select("id", { count: "exact", head: true }).eq("status", "reviewing"),
      context.supabase.from("appeals").select("id", { count: "exact", head: true }).eq("status", "pending"),
      context.supabase.from("moderation_actions").select("id", { count: "exact", head: true }).eq("status", "active"),
    ]);
    return {
      reports_pending: pending.count ?? 0,
